...
COMPANY_CONFIG_DIR = APP_DIR / "config"
COMPANY_PROFILE_EXAMPLE = COMPANY_CONFIG_DIR / "company.example.json"
COMPANY_PROFILE_LOCAL = COMPANY_CONFIG_DIR / "company.local.json"
...
def load_json_file(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning("Invalid JSON file ignored: %s", path)
        return {}


def company_profile() -> dict[str, Any]:
    profile = dict(DEFAULT_COMPANY_PROFILE)
    for path in (COMPANY_PROFILE_EXAMPLE, COMPANY_PROFILE_LOCAL):
        profile = deep_merge_dict(profile, load_json_file(path))
    return profile
...
def on_startup() -> None:
    init_app_db()
    COMPANY_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
...
async def get_company_profile():
    return {"status": "ok", "result": company_profile()}
